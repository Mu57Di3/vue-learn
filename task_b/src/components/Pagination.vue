    ...
    <nav v-if="total > size" aria-label="Page navigation example">
        <ul class="pagination">
            <li :class="['page-item', { disabled: current === 0 }]">
                <a class="page-link" href="#" aria-label="Previous" @click.stop="pageChangeHandler(current - 1)">
            ...
            <li :class="['page-item', { disabled: current === pagesCount - 1 }]" @click="pageChangeHandler(current + 1)">
...
export default {
    ...
    methods: {
        pageChangeHandler: function(newPage) {
